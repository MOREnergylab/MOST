...
if 1
%% Output
Outpufile_name='Mooring_NREL15MW_Spar';
%% Line
rho_water=1025;                                % Water density [kg/m3]
gravity=9.80665;                               % [m/s2]
depth=320;                                     % water depth [m]
d=0.160;                                       % lines diameter [m]
linear_mass_air=460;                           % linear weight in air [kg/m]
...
Data_moor.number_lines=3;                      % Number of lines (angularly equispaced)
                      ...
                      %x        y       z
Data_moor.nodes=     [-9        0      -12;    % Fairlead position (first line)
                      -730      0     -inf]';  % Anchor position (firt line, -inf means water depth)

Data_moor.L=825;                               % Lines unstretched length

Data_moor.EA=2.30e9;                           % Lines stiffness
...
%% Mooring LUT
moor_LUT.X=-20:5:40;                         % Surge positions at which mooring loads are computed
moor_LUT.Y=-15:5:15;                         % Sway positions at which mooring loads are computed
moor_LUT.Z=-15:5:10;                         % Heave positions at which mooring loads are computed
moor_LUT.RX=deg2rad(-5:2.5:5);               % Roll rotations at which mooring loads are computed
