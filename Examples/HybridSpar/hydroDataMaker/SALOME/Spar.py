...
if(GUI!=True):
    ...
    Rupper=PlatformData['Rupper']                         # 3.25
    Rlower=PlatformData['Rlower']                         # 4.7
    Hupper=PlatformData['Hupper']                         # 14
    Hlower=PlatformData['Hlower']                         # 108
    Hcone=PlatformData['Hcone']                           # 8
    Freeboard=PlatformData['Freeboard']                   # 10
    t=PlatformData['t']                                   # 0.02
    Perc_waterBallast=PlatformData['Perc_waterBallast']   # 70
    Rotation_z=PlatformData['Rotation_z']                 # 0
    Maxsize_Nemoh=PlatformData['Maxsize_Nemoh']           # 6
    Minsize_Nemoh=PlatformData['Minsize_Nemoh']           # 3
    Platform_Name=PlatformData['Platform_Name']           # 'Spar5MW'
    ...
    Freeboard=10
    t=0.02
    Perc_waterBallast=70
    Rotation_z=0
    Maxsize_Nemoh=6
    Minsize_Nemoh=3
    Platform_Name='Spar5MW'
    Weight=0    
...
Upper_piece = geompy.MakeCylinderRH(Rupper, Hupper)
geompy.TranslateDXDYDZ(Upper_piece, 0, 0, Freeboard-Hupper)
T_cone = geompy.MakeConeR1R2H(Rlower, Rupper, Hcone)
geompy.TranslateDXDYDZ(T_cone, 0, 0, Freeboard-Hcone-Hupper)
Lower_piece = geompy.MakeCylinderRH(Rlower, Hlower)
geompy.TranslateDXDYDZ(Lower_piece, 0, 0, Freeboard-Hcone-Hupper-Hlower)
Solid = geompy.MakeFuseList([Upper_piece, T_cone, Lower_piece], True, True)
[Shell_Solid] = geompy.ExtractShapes(Solid, geompy.ShapeType["SHELL"], True)
SWL = geompy.MakeCylinderRH(100000, 10000)
...
geompy.addToStudy( Upper_piece, 'Upper_piece' )
geompy.addToStudy( Lower_piece, 'Lower_piece' )
geompy.addToStudy( T_cone, 'T_cone' )
...
### 
MassSpar,COGSpar=writeMassInertia(Shell_Solid ,"Shell_Solid",OUT,steel_density,t)

BallastMass=SubmergedVolume*water_density-MassSpar-Weight
if(BallastMass<0):
    Negative_BallastMass=1
else:
    Negative_BallastMass=0

H_SolidBallast=BallastMass*(1-Perc_waterBallast/100)/solid_ballast_density/np.pi/Rlower**2
SolidBallast = geompy.MakeCylinderRH(Rlower, H_SolidBallast)
geompy.TranslateDXDYDZ(SolidBallast, 0, 0, Freeboard-Hcone-Hupper-Hlower)
SolidBallastMass,COGSolidBallast=writeMassInertia(SolidBallast ,"SolidBallast",OUT,solid_ballast_density,0)

H_WaterBallast=BallastMass*(Perc_waterBallast/100)/water_density/np.pi/Rlower**2
WaterBallast = geompy.MakeCylinderRH(Rlower, H_WaterBallast)
geompy.TranslateDXDYDZ(WaterBallast, 0, 0, Freeboard-Hcone-Hupper-Hlower+H_SolidBallast)
WaterBallastMass,COGWaterBallast=writeMassInertia(WaterBallast ,"WaterBallast",OUT,water_density,0)

M=np.array([MassSpar,SolidBallastMass,WaterBallastMass])
com=np.vstack([COGSpar,COGSolidBallast,COGWaterBallast])
...
geompy.addToStudy( WaterBallast,'WaterBallast')
