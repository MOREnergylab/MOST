...
if 1
    ...
    data.env.depth=320;%ocean depth
    %% GEOMETRY
    if 1
    %% Locations
    data.geom.locations=[0  0 0 ;% pos_x -- pos_y -- rot_z
                         0  0 0].*[1 1 pi/180];
    %% Body 1
    data.geom.Body1.Rupper=5;
    data.geom.Body1.Rlower=9;
    data.geom.Body1.Hupper=25;
    data.geom.Body1.Hlower=155;
    data.geom.Body1.Hcone=25; 
    data.geom.Body1.Freeboard=15;
    ...
    data.geom.Body1.Platform_Name='Spar15MW'; 
    data.geom.Body1.PythonScriptName='Spar.py';
    %% Body 2
    data.geom.Body2.R_ext=17.5;
    data.geom.Body2.H_cyl=7;
    data.geom.Body2.R_int=5.3;
    data.geom.Body2.Draft=3.5;
    data.geom.Body2.t=0.025;
    data.geom.Body2.Maxsize_Nemoh=4;
    data.geom.Body2.Minsize_Nemoh=2;
    data.geom.Body2.Platform_Name='Toroid';
    data.geom.Body2.PythonScriptName='Toroid.py';
    end
    %% TURBINE DATA
    ...
    data.mooring.Body1.filepath=[fileparts(pwd) filesep 'mostData' filesep 'mooring' ...
        filesep 'Mooring_NREL15MW_Spar.mat'];

    %% SALOME
    data.salome.SALOME_path=['C:', filesep, 'SALOME-9.9.0'];   
    %% NEMOH
    data.nemoh.frequencies=[68 2*pi*[0.05  1]]; % rad/s
    data.nemoh.wavedir=[1 0 0];
    data.nemoh.symmetric=0; % flag (0/1)
    data.nemoh.results_folder_name='HybridSpar'; 
    %% BEMIO
