...
plot_flags.PTOs=[1 1 1 1]; % Reactive Power - Active Power - Total Power - Mean total Power
...
%% PTOs
for i=1:length(output.ptos)
    if any(plot_flags.PTOs)
        figure()
        grid on
        title(['PTO' num2str(i)])
        xlabel('Time (s)')
        ...
        if plot_flags.PTOs(1)
            hold on
            plot(output.ptos(i).time,output.ptos(i).velocity(:,3).^2*pto(i).stiffness/1e6,'linewidth',2);
            leg{leg_idx}='Reactive Power'; %#ok<*SAGROW>
            leg_idx=leg_idx+1;
        end

        if plot_flags.PTOs(2)
            hold on
            plot(output.ptos(i).time,output.ptos(i).velocity(:,3).^2*pto(i).damping/1e6,'linewidth',2);
            leg{leg_idx}='Active Power';
            leg_idx=leg_idx+1;
        end

        if plot_flags.PTOs(3)
            hold on
            plot(output.ptos(i).time,output.ptos(i).velocity(:,3).^2*pto(i).stiffness/1e6+...
                output.ptos(i).velocity(:,3).^2*pto(i).damping/1e6,'linewidth',2);
            leg{leg_idx}='Total Power';
            leg_idx=leg_idx+1;
        end
        
        if plot_flags.PTOs(4)
            hold on
            plot(output.ptos(i).time,mean(output.ptos(i).velocity(:,3).^2*pto(i).stiffness/1e6+...
                output.ptos(i).velocity(:,3).^2*pto(i).damping/1e6)*ones(length(output.ptos(i).time),1),'linewidth',2);
            leg{leg_idx}='Mean Total Power';
            leg_idx=leg_idx+1;
        end
